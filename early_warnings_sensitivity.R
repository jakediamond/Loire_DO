...
# Load solute data up to the chlorophyll breakpoint
df <- readRDS("Data/Loire_DO/middle_loire_wq") %>%
  ...
  filter(!(solute == "SPM" & year < 1985),
         !(solute == "BOD5" & year < 1987)) %>%
  mutate(value = if_else(solute == "CHLA", month_mean / 1000, month_mean)) %>%
  # left_join(readRDS("Data/dampierre_discharge_daily_filled") %>%
  #             mutate(year = year(date),
  #                    month = month(date)) %>%
  #             group_by(year, month) %>%
  #             summarize_all(.funs = median) %>%
  #             select(-date), by = c("year", "month")) %>%
  # mutate(value = month_mean * discharge.daily) %>%
  group_by(solute) %>%
  nest() %>%
  ungroup() %>%
  rename(name = solute)
...
ews <- df_ew %>%
  mutate(ts_dat = map(data, ts_conv),
         ew = map(ts_dat, ~generic_ews(.x, winsize = 12)))
...
df_ew_sens_brk1 <- df_ew %>%
  mutate(ts_dat = map(data, ts_conv))

sens_BOD5 <- sensitivity_ews(pluck(df_ew_sens_brk1, 3, 4), 
                             detrending = "gaussian",
                             bandwidthrange = c(1, 11),
                             incrbandwidth = 2,
                             indicator = "ar1",
                             winsizerange = c(10, 50),
                             incrwinsize = 20)
BOD5 <- generic_ews(pluck(df_ew_sens_brk1, 3, 5), 
                    winsize = 50)
sens_BOD5 <- sensitivity_ews(pluck(df_ew_sens_brk1, 3, 5), 
                             indicator = "ar1",
                             winsizerange = c(10, 50),
                             detrending = "first-diff",
                             incrwinsize = 20)
