...
flush_fun <- data.frame(times = times,
                        import =  c(floor(runif(365*4, 1, 9))/10,
                                    floor(runif(365*4, 1, 8))/10,
                                    floor(runif(365*2, 1, 8))/10,
                                    # floor(runif(365*3, 1, 9))/10,
                                    floor(runif(365*2, 1, 6))/10,
                                    floor(runif(365*2, 1, 5))/10,
...
flush_fun <- data.frame(times = times,
                        import =  c(floor(runif(365*4, 1, 9))/10,
                                    floor(runif(365*4, 1, 8))/10,
                                    floor(runif(365*4, 1, 6))/10,
                                    round(runif(365*5, 1, 3))/10,
                                    round(runif(365*4, 1, 2))/10,
                                    runif(365*4, 1, 2)/10))

flush_fun <- data.frame(times = times,
                        import =  c(floor(runif(365*5, 1, 9))/10,
                                    floor(runif(365*4, 1, 8))/10,
                                    floor(runif(365*3, 1, 6))/10,
                                    round(runif(365*4, 1, 3))/10,
                                    round(runif(365*4, 1, 2))/10,
                                    runif(365*5, 1, 2)/10))
...
p_nep_ew <- ggplot() + 
  geom_line(data = nep_p,
            aes(x = time, y = value,
                color = flush)) +
  scale_x_continuous(breaks = seq(1,9125,365),
                     labels = seq(1, 25, 1)) +
  annotate(geom = "rect",
           xmin = 4015,
           xmax = 4380,
...
ggsave(plot = p_nep_ew,
       filename = "Figures/Figure4_final_LandO_early_warnings.png",
       device = "png",
       dpi = 300,
       width = 88,
       height = 120,
