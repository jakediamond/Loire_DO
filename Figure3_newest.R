...
df_dt_raw <- read_excel("Data/all_corrected_rawdata_models for detrend_residuals.xlsx", sheet = 8)
...
# Convert do/chla detrended data to time series format for summer and calculate ew
df_do_ew <- df_do_dt %>%
  # select(1:8) %>%
  as.data.frame(.) %>%
  # filter(between(month(date), 4, 10)) %>%
...
met_plot_data <- df_dt_ew %>%
  ...
  right_join(df_ind,
            by = c("timeindex",
                   "flux")) %>%
  pivot_longer(cols = -c(flux, timeindex, date)) %>%
  mutate(name = case_when(#flux == "NEP" & name == "sd" ~ "cv",
                          #flux == "NEP" & name == "cv" ~ "sd",
                          name == "value" ~ flux, 
                           TRUE ~ name),
         value = case_when(flux == "NEP" & name == "sk" ~-value,
                           TRUE ~ value)) #want to show SD for NEP along with CV of others
...
met_plot_data$plotname <- factor(met_plot_data$plotname,
                            levels = c("NEP~(g~O[2]~m^{-2}~d^{-1})",
                                       "ar(1)",
                                       "standard~deviation",
                                       "skewness"))
levels(met_plot_data$plotname) <- c("NEP~(g~O[2]~m^{-2}~d^{-1})",
                                  "ar(1)",
                                  "standard~deviation",
                                  "skewness")
...
chla_do_plot_data <- chla_do_plot_data %>%
...
# Plot the data
p_ew <- ggplot() + 
  geom_line(data = filter(met_plot_data,
                          flux == "Chla_Res",
                          name %in% c("ar1", "sk", "sd", "NEP")),
            aes(x = date, y = value, color = flux)) +
  scale_x_datetime(date_breaks = "3 years",
               limits = c(ymd_h("1993-01-01 00"), ymd_h("2019-01-01 00")),
               date_labels = "%Y") +
  annotate(geom = "rect",
           xmin = ymd_h("2011-07-01 00"),
           xmax = ymd_h("2012-01-01 00"),
           ymin = -Inf,
           ymax = Inf,
           alpha = 0.4,
           fill = "dark blue") +
  ...
  #                    # breaks = c("Amplitude", "GPP", "NEP"),
  #                    # values = c("black", "dark blue", "dark green"),
  #                    # labels = c("DO_amp", "GPP", "NEP")
  #                    ) +
...
p_ew
  ...
  ggsave(plot = p_ew,
         filename = "Figures/Middle_Loire/Figure3_NEP_svg.svg",
         device = "svg",
         dpi = 300,
         width = 91,
         height = 100,
