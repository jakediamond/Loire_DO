...
df_met <- read_excel("Data/all_corrected_rawdata_models for detrend_residuals.xlsx", sheet = 7) %>%
  mutate(month = month(date),
         year = year(date)) %>%
  select(date, year, month, GPP = GPP_Res, ER = ER_res)

# Define groups
year_grps <- tibble(year = seq(1993, 2018, 1),
                    time = ntile(year, 5))

df_ccm_n_groups <- df_met %>%
...
data = pluck(df_ccm_n_groups, 2, 5)
...
ccm_fun <- function(data){
  Accm = data %>%
    group_by(year) %>%
    nest() %>%
    transmute(clean = future_map(data, ~na_kalman(na.trim(.$GPP)))) %>%
    map_dfr(rbind, NA) %>%
    unnest(clean) %>%
    ungroup(.) %>%
    select(clean) %>%
  ...
  Bccm = data %>%
    group_by(year) %>%
    nest() %>%
    transmute(clean = future_map(data, ~na_kalman(na.trim(.$ER)))) %>%
    map_dfr(rbind, NA) %>%
    unnest(clean) %>%
    ungroup(.) %>%
    select(clean) %>%
...
# ccm results
ccmres_year_groups <- df_ccm_n_groups %>%
  mutate(ccm = future_map(data, ccm_fun))

# saveRDS(ccmres_year_summer, "Data/Loire_DO/ccm_results_groups_detrended")
# saveRDS(ccmres_year, "Data/Loire_DO/ccm_results_years_detrended")
# saveRDS(ccmres_year_groups, "Data/Loire_DO/ccm_results_groupedyears_detrended_summer")
# ccmres <- readRDS("Data/Loire_DO/ccm_results_groupedyears")
# ccmres_year_summer <- readRDS("Data/Loire_DO/ccm_results_years_summer_detrended")
test_year_summer <- unnest_wider(ccmres_year_groups, ccm)

resultsa <- test_year_summer %>% 
  select(time, boota) %>%
  unnest(boota)

resultsb <- test_year_summer %>% 
  select(time, bootb) %>%
  # unnest_longer(bootb) %>%
  unnest(bootb)
pvals <- test_year_summer %>%
  select(time, results) %>%
  unnest(results)
(ggplot(data = pivot_longer(pvals, cols = c(-time)),
    ...
    scale_color_manual(name = "cross map direction",
                       labels = c("GPP causes ER",
                                  "ER causes GPP"),
    ...
    ylab("CCM test p-value") +
    xlab("")) %>%
  ggsave(filename = "Figures/Middle_Loire/supplementary/ccm_groups_detrend_summer.tiff",
         dpi = 300,
         device = "tiff",
         width = 93,
         height = 93,
...
ggplot() +
  ...
  geom_text(data = pvals,
            aes(x = 250, y = 0.96,
                label = paste0("list(","GPP~causes~ER,~p==", a_cause_b, ")")),
            parse = TRUE,
            size = 2.5) +
  geom_text(data = pvals,
            aes(x = 250, y = 0.92,
                label = paste0("list(","ER~causes~GPP,~p==", b_cause_a, ")")),
            parse = TRUE,
            size = 2.5,
            color = "red") +
  facet_wrap(~time) +
...
plot(CCM_boot_A$Lobs, CCM_boot_A$rho, type="l", col=1, lwd=2,
     xlim=c(plotxlimits[1], plotxlimits[2]), ylim=c(0.0,1),
     xlab="L", ylab="rho")
