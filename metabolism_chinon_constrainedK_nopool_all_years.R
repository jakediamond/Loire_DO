# 
# Purpose: To estimate metabolism at Chinon in 2008-2018
# Author: Jake Diamond
...
df$solar.time <- calc_solar_time(df$datetime, longitude = 0.2)
...
df2 <- df %>%
  filter(year(solar.time) > 2007) %>%
  mutate(year = year(solar.time),
         time_frame = as.numeric(cut(year, 2))) %>%
...
df_n <- df2 %>%
  ...
  nest() %>%
  left_join(df_q %>%
              mutate(time_frame = as.numeric(cut(year(date), 2))) %>%
              group_by(time_frame) %>%
              nest() %>%
              rename(data_q = data))
...
saveRDS(mm_all, "F:/DataLocalePerso/Jake/metab_chinon_veryconstrainedK_nopool_all_years")
# Inspect the model -------------------------------------------------------
mm <- mm_all %>%
...
plot_distribs(bayes_specs, "K600_daily_meanlog")
plot_distribs(mm_all[1,1], "K600_daily")
get_specs(mm_all)
...
saveRDS(results, "Data/Loire_DO/metabolism_results_chinon_all_years_constrainedK_no_pool")
