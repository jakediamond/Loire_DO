...
df_light$datetime <- force_tz(df_light$datetime, "Etc/GMT+1")
...
saveRDS(mm_all, "F:/DataLocalePerso/Jake/metab_extremelyconstrainedK_gppconstrained_all_discharge_bins")
# Inspect the model -------------------------------------------------------
mm <- mm_all %>%
...
saveRDS(mm_save, "Data/Loire_DO/metab_extremelyconstrainedK_gppconstrained_all_discharge_bins")

# Same with process errors
process_error_file <- mm_all %>%
  ungroup() %>%
  mutate(r = map(mm, get_fit),
         errs = map(r, pluck, 2)) %>%
  select(errs) %>%
  unnest(cols = c(errs)) %>%
  select(date, solar.time, err_obs_iid_mean, err_proc_iid_mean) %>%
  left_join(mm_all %>%
              ungroup() %>%
              mutate(preds = map(mm, predict_DO)) %>%
              select(preds) %>%
              unnest(cols = c(preds)))
saveRDS(process_error_file, "Data/Loire_DO/metab_extremelyconstrainedK_gppconstrained_all_discharge_bins_process_errors")


plot(mm_save$K600.daily,mm_save$ER)
plot(mm_save$K600.daily)
