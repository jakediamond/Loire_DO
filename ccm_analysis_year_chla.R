...
# Load data
df_met <- df_met <- read_excel("Data/Chla_GPP_11 stations_Gien to Villandry_et HFCM.xlsx", sheet = 3)
...
pvals <- test %>%
  select(year, results) %>%
  unnest(results)
ggplot(data = pivot_longer(pvals, cols = c(-year)),
       aes(x = year,
           y = value,
           color = name)) +
  # geom_jitter() +
...
edm <- Thrips %>%
  mutate(dims = future_map(data, ~EmbedDimension(dataFrame = ., columns = "GPP", target = "ER",
                                          lib = "1 100", pred = "1 100", showPlot = FALSE)),
         rho_theta = future_map(data, ~PredictNonlinear(dataFrame = ., columns = "GPP", target = "ER",
                                                       lib = "1 100", pred = "1 100", E = 6)))

ccm <- Thrips %>%
  mutate(ccm = future_map(data, ~CCM(dataFrame = ., E = 3, Tp = 0, columns = "GPP",
