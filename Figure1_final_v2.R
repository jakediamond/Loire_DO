...
df_mid_wq <- filter(df, site_no %in% c("04048000", #Dampierre
                                       "04050000", #Orleans later
                                       "04050500" #Orleans early
                                       )) %>%
...
p_lm <- lm(lm_dat$PO4~lm_dat$TP) # PO4 is between 40-60% of TP at low concentrations
summary(p_lm)
...
df_mid_long <- df_mid_wide %>%
  ...
  filter(!(solute == "TP" & value > 0.7)) # filter some major outliers for TP
...
solute_bp_data_n <- df_mid_long %>%
  ...
  ungroup() %>%
  filter(!(solute == "BOD5" & year < 1988), #too much missing data before
         !(solute == "SPM" & year < 1984)) %>% #too much missing data before
  group_by(solute) %>%
...
ts_conv <- function(data, log = TRUE, type = "solute"){
  ...
  if(type == "solute"){
    ...
    dat_ts = filter(data, between(month(date), 4, 10))
  }
...
# Quick prior of breakpoints based on visual inspection
bp_priors <- tibble(solute = c("BOD5", "CHLA", "NO3", "PO4", "SPM"),
                    bp_summer = c("dnorm(110,30)",
                                  "dnorm(170,30)",
                                  "dnorm(165,30)",
                                  "dnorm(150,30)",
                                  "dnorm(130,30)"),
                    inits_summer = c(110, 170, 165, 150, 130),
                    bp_all = c("dnorm(210,30)",
                               "dnorm(280,30)",
                               "dnorm(280,30)",
                               "dnorm(270,30)",
                               "dnorm(280,30)"),
                    inits_all = c(210, 280, 280, 270, 280))

# changepoint function
cpt_fun <- function(data, prior, init) {
  # First set the model structure for breakpoints, which includes
  # a break in mean and variability
  model <- list(
    V1 ~ 1 + sigma(1),  
    ~ 1 + sigma(1)
  )
  # The model
  mcp(model = model, data = data, 
      prior = list(cp_1 = prior), 
      inits = list(cp_1 = init),
      par_x = "ind",
      cores = 5,
      chains = 5,
      iter = 10000,
...
# Doing this with map or pmap is not consistent in convergence of Rhat
# but results done individually (see below) align, regardless
solute_bps <- solute_bp_data_n %>%
  left_join(bp_priors) %>%
  mutate(data_summer = map(data, ~na.trim(filter(., between(month(date), 4, 10)))),
         ts = map(data_summer, ts_conv),
         fits = pmap(list(data = ts, prior = bp_summer, init = inits_summer), # for some reason this doesn't work well in purrr::map. better to do individually
                     .f = cpt_fun),
         mcp_res = map(fits, summary))

# NO3, SPM, BOD5 didn't converge with pmap, so redo here individually
model <- list(
  V1 ~ 1 + sigma(1),  
  ~ 1 + sigma(1)
)
# NO3 first
# no3_dat <- pluck(solute_bps, 8, 2)
# cpt.meanvar(no3_dat$V1) # double check with another method cpt = 164 = 2003-09-01
# no3_bp <- mcp(model = model, data = no3_dat, 
#     prior = list(cp_1 = "dnorm(165,30)"), 
#     inits = list(cp_1 = 165),
#     par_x = "ind",
#     cores = 5,
#     chains = 5,
#     iter = 10000,
#     adapt = 10000)
# no3_sum <- summary(no3_bp) %>%
#   mutate(solute = "NO3")
# plot(no3_bp)
# Now SPM/TSS
SPM_dat <- pluck(solute_bps, 8, 4)
cpt.meanvar(SPM_dat$V1) # double check with another method cpt = 132 = 2002-09-01
# Super inconsistent results on Rhat for SPM, but the changepoint mean is always similar
# try with just a model of mean change because variance stays high
SPM_bp <- mcp(model = model,
              data = SPM_dat, 
              prior = list(cp_1 = "dnorm(132,20)"), 
              inits = list(cp_1 = 132),
              par_x = "ind",
              cores = 5,
              chains = 5,
              iter = 10000,
              adapt = 10000)
SPM_sum <- summary(SPM_bp) %>%
  mutate(solute = "SPM")
plot(SPM_bp)
# Now BOD5, same issue here with variance
BOD5_dat <- pluck(solute_bps, 8, 5)
cpt.meanvar(BOD5_dat$V1) # double check with another method cpt = 111 = 2003-09-01
BOD5_bp <- mcp(model = model,
               data = BOD5_dat, 
               prior = list(cp_1 = "dnorm(110,30)"), 
               inits = list(cp_1 = 110),
               par_x = "ind",
               cores = 4,
               chains = 4,
               iter = 8000,
               adapt = 4000)
BOD5_sum <- summary(BOD5_bp) %>%
  mutate(solute = "BOD5")
plot(BOD5_bp)

# Get the output
solute_bp_out <- solute_bps %>%
  select(solute, mcp_res) %>%
  unnest(mcp_res) %>%
  filter(solute %in% c("CHLA", "PO4", "NO3")) %>% #only ones that worked initially
  bind_rows(BOD5_sum, SPM_sum) %>% #ones done individually
  left_join(select(solute_bps, solute, data_summer)) %>%
...
do_bps <- df_do_rm %>%
  ...
  mutate(data_summer = map(data, ~filter(., between(month(date), 4, 10))),
         ts = map(data, ts_conv, log = FALSE),
...
# new model form accounts for change in mean and variance
model_met <- list(
  value ~ 1 + sigma(1),
  ~ 1 + sigma(1) 
)
...
bp_priors_met <- tibble(key = c("GPP", "ER", "K600.daily", "discharge.daily", "NEP"),
                        bp = c("dnorm(4600,1000)",
                               "dnorm(4600,1000)",
                               "dnorm(4600,1000)",
                               "dnorm(4600,1000)",
                               "dnorm(4600,1000)"))
# Do the bayesian changepoint analysis on summer data (faster than all data, same result)
met_bps <- met_bp_data_n %>%
  ...
  mutate(ts = future_map(data, ts_conv, log = FALSE, type = "met"),
         fits = map2(ts, bp, ~mcp(model = model_met, prior = list(cp_1 = .y),
                                  ...
                                  chains = 5)),
         mcp_res = map(fits, summary))
# Same problem with doing it in pmap style, I think it's a processor issue
# Now BOD5, same issue here with variance
GPP_dat <- pluck(met_bps, 4, 1)
cpt.meanvar(GPP_dat$value) # double check with another method cpt = 4659 = 2014-09-12	
GPP_bp <- mcp(model = model_met,
               data = GPP_dat, 
               prior = list(cp_1 = "dnorm(4600,500)"), 
               inits = list(cp_1 = 4600),
               par_x = "ind",
               cores = 4,
               chains = 4,
               iter = 8000,
               adapt = 4000)
GPP_sum <- summary(GPP_bp) %>%
  mutate(key = "GPP")
plot(GPP_bp)

# ER
ER_dat <- pluck(met_bps, 4, 2)
cpt.meanvar(ER_dat$value) # double check with another method cpt = 4659 = 2014-09-12	
ER_bp <- mcp(model = model_met,
              data = ER_dat, 
              prior = list(cp_1 = "dnorm(4600,500)"), 
              inits = list(cp_1 = 4600),
              par_x = "ind",
...
# Get the output
met_bp_out <- met_bps %>%
  mutate(summer_data = map(data, ~filter(., between(month(date), 4, 10)))) %>%
  select(key, summer_data) %>%
  left_join(bind_rows(GPP_sum, ER_sum)) %>%
  select(key, summer_data, name, mean, upper, lower) %>%
  filter(name == "cp_1") %>%
  pivot_longer(cols = c(-key, -name, -summer_data),
               names_to = "type", values_to = "value") %>%
  mutate(value = round(value)) %>%
  mutate(dates = future_map2(summer_data, value, slice)) %>%
  hoist(dates,
        brkdate = "date") %>%
  ungroup() %>%
  select(-summer_data, -dates, -value, -name) %>%
  pivot_wider(names_from = type, values_from = brkdate)
...
met_cpts <- df_met %>%
  ...
  group_by(name) %>%
  nest() %>%
  mutate(data = map(data, na_kalman),
         ts = map(data, ~as.vector(.$value)),
         cps = map(ts, cpt.meanvar),
         cpt = map(cps, pluck, cpts),
         ests = map(cps, pluck, param.est),
         dates = map2(data, cpt, slice)) %>%
  select(name, dates, ests) %>%
  hoist(dates,
        brkdate = "date") %>%
  ungroup() %>%
...
mac_bp_data <- as.data.frame(df_mac %>%
                               ...
                               ungroup() %>%
                               select(year, area))
summary(cpt.mean(mac_bp_data$area)) #breakpoint is 13 =  2009

# Estimates of mean/se for different periods
mac_bp_values <- mac_bp_data %>%
  mutate(period = if_else(year < 2009, 1, 2),
         area = area / 7200) %>%
  group_by(period) %>%
...
summary(cpt.mean(cor_bp_data$d)) #breakpoint is 23 = 2001
...
p_mac <- ggplot(data = filter(df_mac, species != "Elodea nuttallii", year < 2019),
  ...
  scale_x_continuous(limits = c(1993, 2019),
                     ...
                     expand = expand_scale(add = c(0.18, 0))) +
  ...
  annotate(geom = "text", label = "2009", x = 2010, y = 0.12, 
           size = 2.2) +
  annotate(geom = "text", x = 2015, y = 0.03, 
...
p_c <- ggplot(data = df_cor_sum,
  ...
  geom_errorbar(aes(ymin = density - se,
                    ymax = density + se)) +
  geom_vline(aes(xintercept = ymd("2001-07-01")),
             linetype = "longdash",
             color = "black",
             size = 0.5,
             alpha = 0.8) +
...
p_mag_mac <- ggplot(data = mac_bp_values,
  ...
  geom_errorbar(aes(ymin = mean - qt(1 - (0.05 / 2), (sd/se)^2 - 1) * se, 
                    ymax = mean + qt(1 - (0.05 / 2), (sd/se)^2 - 1) * se),
                width = 0.1) +
  geom_blank(data = mutate(mac_bp_values, fake_value = 0.125), 
             aes(y=fake_value)) +
  geom_text(data = mac_bp_values,
            aes(x = period,
                y = mean * 1.5,
                label = paste0("list(",
                               signif(mean, 1),
...
save.image("Figures/Middle_Loire/Figure1_R_environment_data_final_v2.RData")
...
(((p_solutes|p_mag_sol) +plot_layout(widths = c(6,1))) / 
  ...
  ggsave(filename = "Figures/Middle_Loire/Figure1_final_CI_v2_no_color.svg",
         device = "svg",
         dpi = 300,
         height = 250,
         width = 182,
