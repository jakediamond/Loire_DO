...
df_met1 <- readRDS("Data/Loire_DO/metab_extremelyconstrainedK_gppconstrained_all_discharge_bins") %>%
  ungroup() %>%
...
df_do_ew <- df_do_dt %>%
  as.data.frame(.) %>%
  filter(between(month(date), 4, 10)) %>%
  pivot_longer(cols = c(do_amp_res, do_min_res, chla_res), names_to = "flux") %>%
  group_by(flux) %>%
  nest() %>%
