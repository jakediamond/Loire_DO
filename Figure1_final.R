...
mac_bp_data <- as.data.frame(df_mac %>%
                   ...
                   ungroup() %>%
                   select(year, area))
summary(cpt.mean(mac_bp_data$area)) #breakpoint is 2009
...
p_mag_mac <- ggplot(data = mac_bp_values,
                    aes(x = as.factor(period),
                        y = mean)) +
  geom_point(shape = 1, color = "dark green") +
  geom_errorbar(aes(ymin = mean - qt(1 - (0.05 / 2), (sd/se)^2 - 1) * se, 
                    ymax = mean + qt(1 - (0.05 / 2), (sd/se)^2 - 1) * se),
                width = 0.1,
                color = "dark green") +
...
load("Figures/Middle_Loire/Figure1_R_environment_data_final.RData")
...
(((p_solutes|p_mag_sol) +plot_layout(widths = c(6,1))) / 
  ...
  ggsave(filename = "Figures/Middle_Loire/Figure1_final_CI.svg",
         device = "svg",
         dpi = 300,
         height = 250,
         width = 183,
